...
const sendDraftMessage = () => {
  let recieversEmail = document.querySelector('#draft-receipient').value;
  const subject = document.querySelector('#draft-subject').value;
  const message = document.querySelector('#draft-text-area').value;
  const status = 'sent';
  recieversEmail = recieversEmail.trim();
  ...
  if (valid) {
    ...
    fetch(`${baseUrl}/api/v1/messages`, {
      ...
      .then((res) => {
        if (res.status === 201) {
          createContent();
          document.querySelector('#draft-receipient').value = '';
          document.querySelector('#draft-subject').value = '';
          document.querySelector('#draft-text-area').value = '';
          const str = res.message;
          const head = 'SUCCESS';
          const type = 'success';
...
const saveDraft = () => {
  ...
  const validate = () => {
    ...
    const re = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
    if (recieversEmail !== '' && !re.test(recieversEmail)) {
      const str = 'receipient must be a valid email';
      openModal(str);
      setTimeout(() => {
        closeModal();
      }, 4000);
